...
void		draw_sprites(t_box *box)
{
	...
	{
		if (box->sprites.spt[box->sprites.spriteOrder
		...
		{
			box->dsprite.texX = (int)(256 * (box->dsprite.drawStartX -
				(-box->dsprite.spriteWidth / 2 + box->dsprite.spriteScreenX)) *
				64 / box->dsprite.spriteWidth) / 256;
...
{
	box->dsprite.spriteX = box->sprites.spt[box->sprites.spriteOrder
		[box->dsprite.i]].x - box->cam.position.x;
	box->dsprite.spriteY = box->sprites.spt[box->sprites.spriteOrder
		[box->dsprite.i]].y - box->cam.position.y;
	box->dsprite.invDet = 1.0 / (box->cam.p.x * box->cam.d.y -
		box->cam.d.x * box->cam.p.y);
	box->dsprite.transformX = box->dsprite.invDet * (box->cam.d.y *
		box->dsprite.spriteX - box->cam.d.x * box->dsprite.spriteY);
	box->dsprite.transformY = box->dsprite.invDet * (-box->cam.p.y *
		box->dsprite.spriteX + box->cam.p.x * box->dsprite.spriteY);
	box->dsprite.spriteScreenX = (int)((WIND_W / 2) *
		(1 + box->dsprite.transformX / box->dsprite.transformY));
...
void		draw_sprites3(t_box *box)
{
	...
	box->dsprite.drawStartX = -box->dsprite.spriteWidth
	/ 2 + box->dsprite.spriteScreenX;
	if (box->dsprite.drawStartX < 0)
	{
		box->dsprite.drawStartX = 0;
	}
	box->dsprite.drawEndX = box->dsprite.spriteWidth
	/ 2 + box->dsprite.spriteScreenX;
...
void		draw_sprites4(t_box *box)
{
	box->dsprite.d = 0;
	box->dsprite.y = 0;
	if (box->dsprite.transformY > 0 && box->dsprite.drawStartX > 0 &&
		box->dsprite.drawStartX < WIND_W
	&& box->dsprite.transformY < box->sprites.ZBuffer[box->dsprite.
		drawStartX] && box->sprites.ZBuffer[box->dsprite.drawStartX] < INT_MAX)
	{
		box->dsprite.y = box->dsprite.drawStartY;
		while (box->dsprite.y < box->dsprite.drawEndY)
		{
			box->dsprite.d = (box->dsprite.y * 256 - WIND_H *
				128 + box->dsprite.spriteHeight * 128 *
				((-box->go.lop + 1))) - box->ogo.lop * 256;
			box->dsprite.texY = ((box->dsprite.d * 64) /
				box->dsprite.spriteHeight) / 256;
			box->dsprite.texY = abs(box->dsprite.texY) % 64;
			box->dsprite.color = get_pixel(box, box->sprites.tex_sprite[box->
			sprites.spt[box->sprites.spriteOrder[box->dsprite.i]].n_text],
				box->dsprite.texX, box->dsprite.texY);
			if (box->dsprite.color != 0)
				box->pixels[box->dsprite.y * WIND_W +
					box->dsprite.drawStartX] = box->dsprite.color;
			box->dsprite.y++;
		}
	}
}
